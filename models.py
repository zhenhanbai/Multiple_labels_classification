...
class FastTextModel(nn.Module):
    def __init__(self, embedding_file='embedding_English.npz'):
        ...
        self.fc = nn.Linear(self.embedding_size, self.class_num)
...
class RNNModel(nn.Module):
    def __init__(self, embedding_file='embedding_English.npz', \
                 rnn_type="lstm", hidden_dim=256, n_layers=2, bidirectional=True, dropout=0.3, batch_first=True):
        ...
        if self.bidirectional:
            self.fc = nn.Linear(self.hidden_dim * 2, self.class_num)
        else:
