...
max_vocab_size = 8000  # 词表长度限制
train_file = "data/data.txt"  # 要读取的训练文本文件
...
filename_trimmed_file = "embedding_English"  # 用于存储词嵌入向量的文件
...
with open(train_file, 'r', encoding='UTF-8') as f:
    for line in f:
        lin = line.strip()
        if not lin: continue
        content = lin.split('\t')[1].split(" ")  # 分别得到类别和新闻标题
        for word in content:  # 对于新闻标题，以字为单位构建词表
